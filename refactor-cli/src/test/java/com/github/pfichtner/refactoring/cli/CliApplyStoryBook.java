...
/**
 * Approval gallery for all CLI apply scenarios.
 *
 * Each test exercises one subcommand in write mode. The contents of every
 * created, modified, or deleted file are absorbed into
 * {@link CliTestBed#apply(String...)} so the method body is just
 * {@code Approvals.verify(bed.apply(...))}.
 */
@ExtendWith(CliTestBedExtension.class)
class CliApplyStoryBook {
    ...
    @Test
    @CliFixture(root = "fixtures/projects/rename-method/pom.xml")
    void rename_method_by_method_name(CliTestBed bed) throws Exception {
        Path calcFile = bed.root().resolve("src/main/java/com/example/Calculator.java");
        Approvals.verify(bed.apply(
                "rename",
                "--file", calcFile.toString(),
    ...
    @Test
    @CliFixture(root = "fixtures/projects/remove-method/pom.xml")
    void remove_method_no_cascade(CliTestBed bed) throws Exception {
    ...
    void rename_package(CliTestBed bed) throws IOException {
        Approvals.verify(bed.apply(
                "rename-package",
    ...
    void move_class(CliTestBed bed) throws IOException {
