 ...
 */
@CliTestBed(root = "fixtures/projects/convert-to-record/pom.xml")
class CliConvertToRecordTest {
    ...
    void dry_run_prints_preview_without_writing(CommandLine cli, StringWriter out, Path root) throws Exception {
        Path pointFile = root.resolve("src/main/java/com/example/Point.java");
    ...
    @Test
    void rejects_class_with_extends_and_returns_error(CommandLine cli, StringWriter out, Path root) throws Exception {
        Path derivedFile = root.resolve("src/main/java/com/example/Derived.java");
