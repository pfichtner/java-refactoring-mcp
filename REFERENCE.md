...
A set of refactoring tools plus one meta tool are exposed:
...
```
refactor-mcp/target/refactor-mcp-0.1.3-SNAPSHOT-fat.jar
```
...
```bash
claude mcp add java-refactoring -- java -jar /absolute/path/to/refactor-mcp-0.1.3-SNAPSHOT-fat.jar
```
...
The agent sees a set of refactoring tools plus `list_refactorings` once the server is connected:
